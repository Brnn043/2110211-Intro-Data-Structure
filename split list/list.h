...
class list
{
  ...
  public:
    ...
    void splitList(list<T>& list1, list<T>& list2) {
      // Add your code here
      int sz1 = (mSize+1)/2;
      auto nd = mHeader->next;
      for(int i=0;i<sz1;i++) nd=nd->next;

      list1.mHeader->prev->next = mHeader->next;
      mHeader->next->prev = list1.mHeader->prev;

      list1.mHeader->prev = nd->prev;
      nd->prev->next = list1.mHeader;

      list2.mHeader->prev->next = nd;
      nd->prev = list2.mHeader->prev;

      list2.mHeader->prev = mHeader->prev;
      mHeader->prev->next = list2.mHeader;

      list1.mSize=sz1;
      list2.mSize=(mSize-sz1);
      mSize=0;
