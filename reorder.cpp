...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      ...
      list<T> l;
      int idx=0,now=0,n=mSize;
      auto it = begin(),it2=end();
      while(idx<n) {
        if(idx==pos) it2 = it;
        if(now<selected.size()) {
          if(idx==selected[now]) {
            l.push_back(*it);
            auto tmp = it; tmp++;
            it.ptr->prev->next = it.ptr->next;
      ...
      it2.ptr->prev->next = l.mHeader->next;
      l.mHeader->next->prev = it2.ptr->prev;

      it2.ptr->prev = l.mHeader->prev;
      l.mHeader->prev->next = it2.ptr;
